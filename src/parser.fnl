...
(fn wikitext-body? [raw]
  (and (raw:find "{{" 1 true) (not (mw-expr-body? raw))))

(fn config-varref? [name]
  (or (name:match "^FNC%-") (name:match "^FSE%-")))
...
(fn special-varref? [base pin-lvl]
  (or pin-lvl (= base "FNC-TOTALPRICE") (base:match "^FNC%-TOTAL%-(.+)$")
      (config-varref? base)))

(fn expand-user-macros [raw var-env stack]
  (if (not var-env)
      (tostring (or raw ""))
      (pick-values 1 (: (tostring (or raw "")) :gsub "%$([^%$]+)%$"
                        (fn [name]
...
(set parse-additive (fn [tokens pos var-env parse-cache parsing-stack]
                      (var (lhs p)
                           (parse-multiplicative tokens pos var-env parse-cache
                                                 parsing-stack))
                      (while (tok-binop? tokens p "+-")
                        (let [op (. (at tokens p) :value)
                              (rhs p2) (parse-multiplicative tokens (+ p 1)
                                                             var-env parse-cache
                                                             parsing-stack)]
                          (set lhs [:binop op lhs rhs])
                          (set p p2)))
                       ...
                       (let [eb (extract-mw-expr expanded)]
                         (if eb
                             ...
                             (let [tokens (lex expanded)]
                               ...
                               (let [(ast pos) (parse-additive tokens 1 var-env
                                                               parse-cache stack)]
                                 (when (<= pos (length tokens))
                                   (error (unexpected (at tokens pos))))
...
(fn parse-var-env [var-env]
  (let [parse-cache {}
    ...
    (each [name _ (pairs var-env)]
      (when (and (not (config-varref? name)) (= (. parse-cache name) nil))
