      ...
      (let [c (s:sub i i)]
        (if (c:match "%s")
            ...
            (do
              (var j i)
              (while (and (<= j len)
                          (or (is-digit (s:sub j j)) (= (s:sub j j) ".")))
                (set j (+ j 1)))
              (table.insert tokens
                            {:type :num :value (tonumber (s:sub i (- j 1)))})
              (set i j))
            (is-alpha c)
            (do
              (var j i)
              ...
              (let [word (s:sub i (- j 1))
                ...
                (table.insert tokens
                              (if (or (= lower "e") (= lower "pi")
                                      ...
                                      (= lower "and") (= lower "or")
                                      (= lower "not") (= lower "abs")
                                      (= lower "floor") (= lower "ceil")
                                      (= lower "trunc") (= lower "round")
                                      (= lower "sqrt") (= lower "ln")
                                      (= lower "exp") (= lower "sin")
                                      (= lower "cos") (= lower "tan")
                                      (= lower "asin") (= lower "acos")
                                      (= lower "atan"))
...
(fn parse-eval [source bindings]
  (let [tokens (tokenize source)
      ...
      (let [t (peek)]
        (when (not (and (= t.type typ) (or (= val nil) (= t.value val))))
          (error (.. "mwexpr: expected " (tostring typ)
                     (if val (.. " " val) "") ", got " (tostring t.type))))
        (advance)))
    ...
    (fn parse-primary []
      (let [t (peek)]
        (if (= t.type :num)
            ...
            (let [name t.value]
              (advance)
              (let [v (or (. bindings name) (. bindings (name:lower)))]
                (when (= v nil)
            ...
            (do
              ...
              (let [v (parse-or)]
                (expect :rparen)
                v))
            (error (.. "mwexpr: unexpected token " (tostring t.type) " "
                       (tostring t.value))))))
    ...
    (fn parse-product []
      (var v (parse-power))
      (while (or (at :op "*") (at :op "/") (at :word "div") (at :word "mod"))
    ...
    (let [result (parse-or)]
      (when (not= (. (peek) :type) :eof)
        (error (.. "mwexpr: trailing input near " (tostring (. (peek) :value)))))
      result)))
