...
(fn run []
  ...
  (assert-eq
    (lex "Firework Technician Stats.Firework Shot Chance Cap / (1 + 2.718**(-0.5*(Level)))")
  ...
  (assert-eq (lex "18275 / 36 * 6")
             ...
             "integer arithmetic")
  (assert-eq (lex ".5 + 1.25")
             [(t :num 0.5) (t :op "+") (t :num 1.25)]
             "leading-dot decimal")
  (assert-eq (lex "18,275 + 1_000")
             [(t :num 18275) (t :op "+") (t :num 1000)]
  ...
  (assert-eq (lex "$MCE$ + $TOKEN@5@Branch$ + $FNC-TOTALPRICE$")
             [(t :varref "MCE")
              ...
              (t :op "+")
              (t :varref "FNC-TOTALPRICE")]
             "varref variants")

  (suite "trailing digits stay in ident (DPS2)")
  (assert-eq (lex "DPS2 + x")
             [(t :ident "DPS2") (t :op "+") (t :ident "x")]
             "DPS2 is one ident")
  ...
  (assert-eq (lex "(Cash Shot--)")
             [(t :lparen)
              ...
              (t :rparen)]
             "Cash Shot--")
  (assert-eq (lex "Burst Size++")
             [(t :ident "Burst Size") (t :op "++")]
             "Burst Size++")

  (suite "[[wikilink]] is column ident")
  (assert-eq (lex "[[Splash Damage]] / Firerate")
             [(t :ident "Splash Damage")
              (t :op "/")
              (t :ident "Firerate")]
