...
(local {: parse-level-keys : clean-header-text : split-header-cells
        : split-row-cells : parse-table : find-table-blocks : parse-all-tables
        : detect-branch : index-col : row-level-keys}
       (require :wikitable))
...
(fn run []
  ...
  (let [cells (split-header-cells
                "! Level !! Damage || Cost Efficiency")]
  ...
  (let [t (parse-table thorns-table {})]
    ...
    (assert-eq (. t.rows 1 :by-header "Cost Efficiency")
               "{{Money|$TCE$}}"
               "last cell under correct header")
    (assert-eq (. t.rows 1 :by-header "Thorns DPS")
               "$TDPS$"
               "Thorns DPS cell"))
  ...
  (let [bmap {"Harvester Stats" "N"  ; not a real branch letter usage
              "Thorns Stats" "T"
              "Top Path Stats" "A"}
  ...
  (suite "detect-branch")
  (assert-eq (detect-branch "Top Path Stats" {"Top Path Stats" "A"}) "A" "exact")
  ...
  (assert-eq (split-row-cells "| 0 || 1 || $DPS$")
             ["0" "1" "$DPS$"]
             "three cells")
