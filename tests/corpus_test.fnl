...
(fn classify-output [name out failures]
  (if (or (not out) (= (or (out:match "^%s*(.-)%s*$") "") ""))
      ...
      (let [leftover (first-leftover-var out)]
        (if leftover
            (do
              (table.insert failures [name (.. "unresolved $" leftover "$")])
              :leftover)
            :ok))))
...
(fn run []
  (let [names (list-towers)
    ...
    (print (.. "  ok=" ok-n "  fail=" fail-n "  empty=" empty-n
               "  leftover_VAR=" leftover-n "  leftover_varblock=" varblock-n))
    (when (> (length failures) 0)
      ...
      (when (> (length failures) 40)
        (print (.. "    ... +" (tostring (- (length failures) 40)) " more"))))
