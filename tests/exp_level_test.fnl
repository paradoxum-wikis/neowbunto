...
(fn run []
	...
	(let [env {"EXP" EXP-RAW}
		...
		(let [kwargs (. calls 1 1)]
			...
			(let [expr (. kwargs.args 1)]
				(assert-true (not (expr:find "Level" 1 true)) "Level already materialized")
	...
	(let [env {"EXP" EXP-RAW "TE" "$FNC-TOTAL-EXP$"}
		...
		(each [_ [lvl expected] (ipairs [[0 0] [1 50] [2 104] [3 163] [4 227]])]
			(let [ctx (make-ctx {:config cfg :vars vars :formula-env env
								 :parse-cache cache :formula-asts cache
								 :level lvl :row {"Level" lvl}})]
	...
	(let [env {"EXP" EXP-RAW "TE" "$FNC-TOTAL-EXP$"}
		  cache (parse-var-env env)
		  vars {"$EXP$" EXP-RAW}
		  ctx (make-ctx {:level 2 :row {"Level" 2}
						 :vars vars :formula-env env
						 :parse-cache cache :formula-asts cache
