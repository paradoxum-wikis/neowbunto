...
(fn run []
  ...
  (assert-eq (parse-string "18275/36*6")
             [:binop "*" [:binop "/" [:num 18275] [:num 36]] [:num 6]]
             "18275/36*6")
  (suite "2+3*4: * binds tighter than +")
  (assert-eq (parse-string "2+3*4")
             [:binop "+" [:num 2] [:binop "*" [:num 3] [:num 4]]] "2+3*4")
  ;; ** is right-assoc 2**3**2 = 2**(3**2) = 512 not 64
  (suite "** is right-associative (matches old calc)")
  (assert-eq (parse-string "2**3**2") [:pow [:num 2] [:pow [:num 3] [:num 2]]]
             "2**3**2 => 2**(3**2)")
  (assert-eq (parse-string "(2**3)**2")
             [:pow [:pow [:num 2] [:num 3]] [:num 2]] "(2**3)**2 left-grouped")
  (assert-eq (parse-string "2**(3**2)")
             [:pow [:num 2] [:pow [:num 3] [:num 2]]] "2**(3**2) explicit right")
  ...
  (assert-eq (parse-string "a/b/c")
             [:binop "/" [:binop "/" [:ident "a"] [:ident "b"]] [:ident "c"]]
             "a/b/c")
  (assert-eq (parse-string "a*b%c")
             [:binop "%" [:binop "*" [:ident "a"] [:ident "b"]] [:ident "c"]]
             "a*b%c")
  ...
  (assert-eq (parse-string "1+2*3**2")
             [:binop
              "+"
              [:num 1]
              [:binop "*" [:num 2] [:pow [:num 3] [:num 2]]]]
             "1+2*3**2")
  (suite "parens override precedence")
  (assert-eq (parse-string "(1+2)*3")
             [:binop "*" [:binop "+" [:num 1] [:num 2]] [:num 3]] "(1+2)*3")
  (assert-eq (parse-string "((2))") [:num 2] "nested parens collapse")
  (suite "unary minus binds tighter than **")
  (assert-eq (parse-string "-2**2") [:pow [:unop "-" [:num 2]] [:num 2]]
             "-2**2 => (-2)**2")
