...
(local {: suite : assert-eq : assert-true : assert-error} (require :test_util))

(local {: parse-vars
        : build-config
        : parse-page-config
        : split-semi
        : is-numeric-array-body
        : parse-number-list
        : get-var-key
        : get-array-var-key
        : formula-env
        : config-name?} (require :config))

(local {: eval-node : make-ctx : sum-total} (require :eval))
(local {: parse-with-env : parse-var-env} (require :parser))

(local electroshocker-var "<var>
$DPS$ = Damage / Firerate
$MDPS$ = DPS * Max Hits
$CE$ = Total Price / DPS
$TP$ = $FNC-TOTALPRICE$
...
(fn run []
  ...
  (let [(vars is-fse) (parse-vars electroshocker-var)]
    ...
    (assert-eq (. vars "$FNC-COST$") "375; 200; 700; 2500; 6350; 16935"
               "cost raw")
    ...
    (let [env (formula-env vars)]
      ...
      (assert-eq (. env "DREF") (. vars "$DREF$")
                 "literal ref kept as formula-env entry")))
  ...
  (let [(vars is-fse) (parse-vars branched-var)
    ...
    (assert-eq (. cfg.index 1) {:table "Sentry Stats" :col "Level"} "INDEX")
  ...
  (let [(cfg _) (parse-page-config electroshocker-var "")
    ...
    (assert-eq (eval-node ctx2 [:intrinsic :totalprice]) (+ 375 200 700) "L2"))
  ...
  (let [(cfg _) (parse-page-config branched-var "")
    ...
    (assert-eq (eval-node ctx node) (+ 100 200 50)
